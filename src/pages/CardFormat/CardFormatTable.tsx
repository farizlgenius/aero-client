...
interface TableContent {
    data: CardFormatDto[]
    handleEdit: (data: CardFormatDto) => void
    handleRemove: (data: CardFormatDto) => void
    handleCheck: (data: CardFormatDto, e: React.ChangeEvent<HTMLInputElement>) => void;
    handleCheckAll: (data: CardFormatDto[], e: React.ChangeEvent<HTMLInputElement>) => void;
    selectedObject: CardFormatDto[];
}

export const CardFormatTable:React.FC<PropsWithChildren<TableContent>> = ({ selectedObject, data, handleEdit, handleRemove, handleCheck, handleCheckAll })  => {
    return (
        <>
                ...
                <Table>
                    ...
                    <TableHeader className="border-b border-gray-100 dark:border-white/[0.05] bg-white dark:bg-gray-900 sticky top-0 z-10">
                        <TableRow>
                            ...
                            {CARDFORMAT_TABLE_HEAD.map((head: string, i: number) =>
                                <TableCell
                                    key={i}
                                    isHeader
